...
var find_block = function(height_or_hash, callback) {
  ...
  return Blocks.findOne(conditions, no_id)
    .lean()
    .exec(callback)
}
...
var add_used_txid = function(tx, callback) {
  ...
  tx.ccdata = tx.ccdata || []
  var outputs = tx.vout.map(function(output) {
    return { txid: tx.txid, index: output.n }
  })
...
var find_transaction = function(txid, callback) {
  ...
  return RawTransactions.findOne(conditions, no_id)
    .lean()
...
var find_transactions = function(txids, colored, callback) {
  ...
  return RawTransactions.find(conditions, no_id)
    .sort('time')
    .lean()
  ...
  async.waterfall(
    [
      function(cb) {
        ...
        AddressesTransactions.find({ address: address }, no_id)
          .lean()
          .exec(cb)
      },
      function(address_txids, cb) {
        ...
        async.whilst(
          ...
          function(cb) {
            ...
            RawTransactions.find(conditions, no_id)
              .lean()
              .exec(function(err, bulk_txs) {
                if (err) return cb(err)
          ...
          if ('vout' in tx && tx.vout) {
            tx.vout.forEach(function(vout) {
              if ('scriptPubKey' in vout && vout.scriptPubKey) {
                if (
                  'addresses' in vout.scriptPubKey &&
      ...
      function(cb) {
        ...
        AddressesUtxos.find({ address: address }, no_id)
          .lean()
          .exec(cb)
      },
      function(address_utxos, cb) {
        ...
        Utxos.find({ $or: conditions }, no_id)
          .lean()
          .exec(cb)
      },
...
var count_asset_transactions = function(assetId, type, callback) {
  ...
  AssetsTransactions.count(conditions)
    .lean()
    .exec(callback)
}
  ...
  async.waterfall(
    [
      function(cb) {
        ...
        AssetsTransactions.find(conditions, no_id)
          .lean()
          .exec(cb)
      },
      function(assets_transactions, cb) {
        ...
        RawTransactions.find({ $or: conditions }, no_id)
          .lean()
          .exec(cb)
      },
      function(txs, cb) {
...
var find_asset_first_block = function(assetId, callback) {
  ...
  AssetsTransactions.find(conditions)
    .lean()
    .exec(function(err, assets_transactions) {
      if (err) return callback(err)
      var txids = []
      assets_transactions.forEach(function(asset_transaction) {
        txids.push(asset_transaction.txid)
      })
      RawTransactions.find({ txid: { $in: txids }, blockheight: { $gte: 0 } }, { blockheight: 1, _id: 0 })
        .sort({ blockheight: 1 })
        .limit(1)
        .lean()
...
var find_asset_utxos = function(assetId, confirmations, callback) {
  ...
  async.waterfall(
    [
      function(cb) {
        AssetsUtxos.find({ assetId: assetId }, no_id)
          .lean()
          .exec(cb)
      },
...
var find_blocks = function(start, end, callback) {
  ...
  if (start < 0 && !end) {
    ...
    if (limit > MAX_BLOCKS_ALLOWED) {
      return callback(
        new errors.BlocksRangeTooHighError({
...
var find_asset_info = function(assetId, with_transactions, callback) {
  ...
  async.parallel(functions, function(err, results) {
    ...
    var totalBurned =
      _.sumBy(results.burns, function(burn) {
        return find_burn_amounts(burn)[assetId] || 0
      }) || 0
    ...
    if (!with_transactions) {
      ...
      asset_info.issuances.forEach(function(transaction) {
        if (
          !asset_info.firstBlock ||
          asset_info.firstBlock === -1 ||
          (transaction.blockheight !== -1 && asset_info.firstBlock > transaction.blockheight)
        ) {
...
var find_first_issuance = function(assetId, utxo, callback) {
  ...
  Utxos.findOne(conditions, no_id)
    .lean()
    .exec(function(err, utxo_obj) {
      if (err) return callback(err)
  ...
  async.waterfall(
    ...
    function(err, groups) {
      ...
      _.times(numOfIntervals, i => {
        var group
        var group_index = binarySearch(groups, i, (a, b) => {
          return a._id - b
        })
        if (group_index >= 0) {
  ...
  async.waterfall(
    [
      ...
      function(mempool_txs, cb) {
        ...
        RawTransactions.find(conditions)
          .count()
...
var find_popular_assets = function(sort_by, limit, callback) {
  ...
  collection
    ...
    .sort('-count')
    .limit(limit)
    .exec(function(err, asset_counts) {
      if (err) return callback(err)
      var assets = []
      var assetsOrder = {}
      var i = 0
...
var find_utxo = function(txid, index, callback) {
  Utxos.findOne({ txid: txid, index: index }, no_id)
    .lean()
    .exec(function(err, utxo) {
      if (err) return callback(err)
...
var find_utxos = function(utxos, callback) {
  ...
  Utxos.find({ $or: or }, no_id)
    .lean()
    .exec(function(err, txos) {
      if (err) return callback(err)
...
var find_mempool_txids = function(colored, callback) {
  ...
  RawTransactions.find(conditions, projection)
    .lean()
    .exec(function(err, txs) {
      if (err) return callback(err)
      return callback(
...
var is_active = function(req, res, next) {
  ...
  AddressesTransactions.aggregate({ $match: match }, { $group: group }, { $sort: sort }).exec(function(
    err,
    active_addresses
  ) {
    if (err) return next(err)
    var ans = addresses.map(function(address) {
      var found =
        binarySearch(active_addresses, address, (a, b) => {
          return a._id.localeCompare(b)
        }) >= 0
      return {
        address: address,
        active: found
